...
if [[ ",${INSTALL_BACKENDS}," == *,vllm,* && ",${INSTALL_BACKENDS}," == *,sglang,* ]]; then
  cat <<'EOF'
scripts/setup_env.sh refuses to install vLLM and SGLang into the same environment by default.
...
Use separate environments instead, for example:
  ENV_NAME=verlrun-vllm INSTALL_BACKENDS=vllm bash scripts/setup_env.sh
  ENV_NAME=verlrun-sglang INSTALL_BACKENDS=sglang bash scripts/setup_env.sh
EOF
  exit 2
fi
...
For reproducible vLLM runs, start from a clean env or choose a fresh name, for example:
  ENV_NAME=${ENV_NAME}-vllm INSTALL_BACKENDS=vllm bash scripts/setup_env.sh
EOF
    exit 2
  fi
fi
...
For reproducible SGLang runs, start from a clean env or choose a fresh name, for example:
  ENV_NAME=${ENV_NAME}-sglang INSTALL_BACKENDS=sglang bash scripts/setup_env.sh
EOF
    exit 2
  fi
fi
