    ...
    for value in values:
        for part in value.split(","):
            ...
            if "-" in part:
                ...
                chunks.add(int(part))
    bad = [chunk for chunk in chunks if chunk < 1 or chunk > 20]
    if bad:
        raise SystemExit(f"Video-MME chunks must be in 1..20, got {bad}")
...
def main() -> int:
    ...
    raw_dir = asset_root / "Video-MME"
    cache_dir = Path(args.video_cache_dir).expanduser().resolve() / "videomme"
    chunks = _parse_chunks(args.chunk) if args.chunk else list(range(1, 21))
    ...
    manifest = Path(args.manifest).expanduser() if args.manifest else raw_dir / "videomme_official_video_manifest.json"
