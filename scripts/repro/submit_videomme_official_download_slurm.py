...
#SBATCH --job-name=revise-videomme-official
#SBATCH --partition={partition}
{account_line}#SBATCH --cpus-per-task={cpus_per_task}
#SBATCH --mem={mem}
#SBATCH --time={time_limit}
#SBATCH --array=1-20%{concurrency}
...
python scripts/repro/download_videomme_official_videos.py \\
  --chunk "${{SLURM_ARRAY_TASK_ID}}" \\
  --video-cache-dir {shlex.quote(str(video_cache_dir))} \\
  --manifest "{results_dir}/videomme-official-chunk-${{SLURM_ARRAY_TASK_ID}}.json"{delete_flag}{overwrite_flag}
"""


def main() -> int:
    parser = argparse.ArgumentParser(description="Submit official Video-MME HF video chunk download/extract jobs.")
    ...
    if args.concurrency <= 0 or args.concurrency > 20:
        raise SystemExit("--concurrency must be in 1..20.")

    run_name = args.run_name or "videomme-official-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    ...
    job_path = run_dir / "jobs" / "videomme-official-download.sbatch"
    job_path.write_text(
        _sbatch_script(
            run_dir=run_dir,
