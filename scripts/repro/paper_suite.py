...
from scripts.repro.check_video_cache_coverage import _coverage, _dataset_split
...
def _cmd_videomme_pnp_hf(assets: dict, smoke: bool, out_dir: Path) -> list[str]:
    cmd = [
        _python_bin(),
        str(REPO_ROOT / "examples/revise/plug_and_play_lvbench_hf.py"),
        "--dataset",
        "videomme",
        "--max-samples",
        "1" if smoke else "0",
...
def _manual_videoespresso_pipeline(assets: dict, smoke: bool, out_dir: Path) -> str:
    ...
    return (
        ...
        + shlex.quote(str(python_bin))
        + " "
        + shlex.quote(str(REPO_ROOT / "scripts/repro/extract_videoespresso_split_zip_subset.py"))
        + " "
...
def _lvbench_reward_ablation_builder(config_name: str, exp_label: str):
    ...
    def _build(assets: dict, smoke: bool, out_dir: Path) -> str:
        python_bin = _python_bin()
        config_dir = REPO_ROOT / "examples/revise/config"
